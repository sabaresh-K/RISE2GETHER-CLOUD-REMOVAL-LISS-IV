...
# 3. Clean & High-Contrast Streamlit CSS Theme
ST_THEME_CSS = """
<style>
    /* Dark Theme Base */
    .stApp {
...
# Load image based on selection or upload
if uploaded_file:
    try:
...
with tab_home:
    st.markdown('''
    <div class="info-card">
        <h1 style="color:#00D4FF; font-size:2.2rem; font-weight:800; margin-top:0;">Revealing the Earth Beneath the Clouds</h1>
    ...
    with a1:
        st.markdown('<div class="info-card"><h4 style="color:#00D4FF;">01. Data Acquisition</h4><p style="font-size:0.85rem; color:#94A3B8;">5.8m LISS-IV optical bands + Sentinel-1 C-band SAR radar.</p></div>', unsafe_allow_html=True)
    with a2:
        st.markdown('<div class="info-card"><h4 style="color:#F97316;">02. Preprocessing</h4><p style="font-size:0.85rem; color:#94A3B8;">Geo-alignment, calibration, and cloud/shadow mask extraction.</p></div>', unsafe_allow_html=True)
    with a3:
        st.markdown('<div class="info-card"><h4 style="color:#F59E0B;">03. AI Processing</h4><p style="font-size:0.85rem; color:#94A3B8;">SAR-guided CycleGAN model infuses radar structural features.</p></div>', unsafe_allow_html=True)
    with a4:
        st.markdown('<div class="info-card"><h4 style="color:#10B981;">04. Output Layer</h4><p style="font-size:0.85rem; color:#94A3B8;">GeoTIFF generation with full CRS metadata & telemetry evaluation.</p></div>', unsafe_allow_html=True)
...
with tab_about:
    ...
    with m1:
        st.markdown('<div class="telemetry-box"><div class="telemetry-val">30.2 dB</div><div class="telemetry-lbl">PSNR</div></div>', unsafe_allow_html=True)
    with m2:
        st.markdown('<div class="telemetry-box"><div class="telemetry-val">0.884</div><div class="telemetry-lbl">SSIM</div></div>', unsafe_allow_html=True)
    with m3:
        st.markdown('<div class="telemetry-box"><div class="telemetry-val">4.52°</div><div class="telemetry-lbl">SAM</div></div>', unsafe_allow_html=True)
    with m4:
        st.markdown('<div class="telemetry-box"><div class="telemetry-val">0.032</div><div class="telemetry-lbl">RMSE</div></div>', unsafe_allow_html=True)
    with m5:
        st.markdown('<div class="telemetry-box"><div class="telemetry-val">94.6%</div><div class="telemetry-lbl">Mask Acc</div></div>', unsafe_allow_html=True)
    with m6:
        st.markdown('<div class="telemetry-box"><div class="telemetry-val">3.2s</div><div class="telemetry-lbl">Speed</div></div>', unsafe_allow_html=True)
...
with tab_features:
    ...
    f1, f2 = st.columns(2)
    with f1:
        st.markdown('<div class="info-card" style="border-left:4px solid #00D4FF;"><h4 style="color:#00D4FF;">🛰️ LISS-IV 5.8m Resolution Preservation</h4><p style="color:#94A3B8;">Engineered for ISRO\'s LISS-IV 5.8m pixel spacing without downscaling.</p></div>', unsafe_allow_html=True)
        st.markdown('<div class="info-card" style="border-left:4px solid #10B981;"><h4 style="color:#10B981;">🔄 Multi-Sensor Fusion Engine</h4><p style="color:#94A3B8;">Fuses optical reflection with Sentinel-1 C-band active radar backscatter.</p></div>', unsafe_allow_html=True)
    with f2:
        st.markdown('<div class="info-card" style="border-left:4px solid #F59E0B;"><h4 style="color:#F59E0B;">🔁 CycleGAN Generative Core</h4><p style="color:#94A3B8;">Dual generators ensure realistic, spectrally accurate ground texture synthesis.</p></div>', unsafe_allow_html=True)
        st.markdown('<div class="info-card" style="border-left:4px solid #8B5CF6;"><h4 style="color:#8B5CF6;">📍 Full Georeferenced GeoTIFF Export</h4><p style="color:#94A3B8;">Exports GIS-ready GeoTIFF rasters with intact CRS metadata.</p></div>', unsafe_allow_html=True)

# TAB 4: MODEL PAGE (WORKING INTERACTIVE DASHBOARD)
with tab_model:
    # RUN MODEL Action Button
    ...
    with m1:
        st.markdown(f'<div class="telemetry-box"><div class="telemetry-val">{psnr}</div><div class="telemetry-lbl">PSNR (Peak Signal)</div></div>', unsafe_allow_html=True)
    with m2:
        st.markdown(f'<div class="telemetry-box"><div class="telemetry-val">{ssim}</div><div class="telemetry-lbl">SSIM Index</div></div>', unsafe_allow_html=True)
    with m3:
        st.markdown(f'<div class="telemetry-box"><div class="telemetry-val">{rmse}</div><div class="telemetry-lbl">RMSE Error</div></div>', unsafe_allow_html=True)
    with m4:
        st.markdown(f'<div class="telemetry-box"><div class="telemetry-val">{sam}</div><div class="telemetry-lbl">SAM Spectral Angle</div></div>', unsafe_allow_html=True)
...
with tab_contact:
    ...
    with t1:
        st.markdown('<div class="info-card" style="border-left:4px solid #00D4FF;"><h3 style="color:#00D4FF; margin:0 0 0.3rem 0;">Sabaresh K</h3><p style="color:#94A3B8; font-size:0.85rem; margin-bottom:0.8rem;">AI & Deep Learning Lead</p><a href="mailto:sabaresh.k2025aids@sece.ac.in" style="color:#00D4FF; text-decoration:none;">📧 sabaresh.k2025aids@sece.ac.in</a></div>', unsafe_allow_html=True)
    with t2:
        st.markdown('<div class="info-card" style="border-left:4px solid #00D4FF;"><h3 style="color:#00D4FF; margin:0 0 0.3rem 0;">Saadhana S</h3><p style="color:#94A3B8; font-size:0.85rem; margin-bottom:0.8rem;">GIS & GeoTIFF Specialist</p><a href="mailto:saadhana.s2025aids@sece.ac.in" style="color:#00D4FF; text-decoration:none;">📧 saadhana.s2025aids@sece.ac.in</a></div>', unsafe_allow_html=True)
    with t3:
        st.markdown('<div class="info-card" style="border-left:4px solid #00D4FF;"><h3 style="color:#00D4FF; margin:0 0 0.3rem 0;">Pranika R</h3><p style="color:#94A3B8; font-size:0.85rem; margin-bottom:0.8rem;">UI/UX & Telemetry Specialist</p><a href="mailto:pranika.r2025aids@sece.ac.in" style="color:#00D4FF; text-decoration:none;">📧 pranika.r2025aids@sece.ac.in</a></div>', unsafe_allow_html=True)
